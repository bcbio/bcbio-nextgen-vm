...
def write_version_py():
    version_py = os.path.join(os.path.dirname(__file__), "bcbiovm",
                              "version.py")
    try:
        import subprocess
...
setup(name="bcbio-nextgen-vm",
      ...
      description=("Run bcbio-nextgen genomic sequencing analyses using "
                   "isolated containers and virtual machines"),
      license="MIT",
...
def ansible_pb_files(ansible_pb_dir):
    """Retrieve ansible files for installation. Derived from
    elasticluster setup.
    """
    ansible_data = []
...
if __name__ == "__main__":
    ...
    if sys.argv[1] in ["develop", "install"]:
        for dirname, fnames in (ansible_pb_files("ansible") +
                                elasticluster_config_files("elasticluster")):
